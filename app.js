...
function loadDecisions(){
 try{const s=JSON.parse(localStorage.getItem('converge-demo-decisions')||'{}');return s&&typeof s==='object'?s:{};}catch(e){return {};}
}
let decisions=loadDecisions();
function decisionKey(loanId,title,name){return [loanId,title,name].join('|');}
function getDecision(loanId,title,name){return decisions[decisionKey(loanId,title,name)]||null;}
function persistDecision(loanId,title,name,status,rationale){
 decisions[decisionKey(loanId,title,name)]={status:status,rationale:rationale,reviewer:role,timestamp:new Date().toISOString()};
 localStorage.setItem('converge-demo-decisions',JSON.stringify(decisions));
}
function check(items,title,editable,locked,loanId){
 return '<section class="section"><div class="sectionhead"><div><span class="badge '+(title==='Closing'?'teal':'')+'">'+(title==='Underwriting'?'Station 1':'Station 2')+'</span><h2>'+title+' checklist</h2><p>'+(title==='Underwriting'?'Leverage, coverage, guarantors, experience and guarantor credit.':'Insurance, trust documentation, exceptions, deed and name consistency.')+'</p>'+(locked?'<div class="lock">🔒 Locked until Station 1 is complete</div>':'')+'</div></div>'+items.map(function(x){
 var d=getDecision(loanId,title,x[0]);
 var disp=d?'<div style="margin-top:7px;font-size:9px;color:#566176"><b>Reviewer disposition:</b> '+d.status+' · '+new Date(d.timestamp).toLocaleString()+(d.rationale?' · '+d.rationale:'')+'</div>':'';
 var act=editable&&x[1]!=='ready'&&x[1]!=='na'&&!locked?'<a class="secondary" href="#action/'+title+'/'+encodeURIComponent(x[0])+'">Review / disposition</a>':editable&&locked&&x[1]!=='ready'&&x[1]!=='na'?'<span class="readonly">Locked by upstream underwriting</span>':editable?'<span class="readonly">Ready — no action needed</span>':'<span class="readonly">Read-only · owned by '+title+'</span>';
 return '<div class="item"><div class="itemicon">'+(x[1]==='ready'?'✓':x[1]==='issue'?'!':x[1]==='waiting'?'…':'–')+'</div><div><div class="row"><h3>'+x[0]+'</h3>'+pill(x[1])+'</div><p><b>'+x[2]+'</b></p><p><span class="source">Value: '+x[3]+' · Source: '+x[4]+'</span></p>'+disp+'<div class="actions">'+act+'</div></div></div>';
 }).join('')+'</section>';
}
function detail(L){
...
function actionPage(){
 ...
 var params=new URLSearchParams(location.search||''), loanId=params.get('loan'), L=loanId?getLoan(loanId):null;
 var d=L?getDecision(loanId,title,name):null;
 var current=d?d.status:'';
 var rationale=d?d.rationale:'';
 var opts=['Accepted','Dismissed','Waived','Escalated'].map(function(v){return '<option value="'+v+'" '+(current===v?'selected':'')+'>'+v+'</option>';}).join('');
 document.getElementById('app').innerHTML=top(role==='station1'?'Underwriting':'Closing',role==='station1'?'Station 1':'Station 2')+'<main><a class="back" href="'+(L?'loan.html?loan='+L.id+'&station='+(role==='station1'?'1':'2'):'#'+role)+'">← Back to loan</a><section class="section"><div class="sectionhead"><div><div class="eyebrow">REVIEW FINDING'+(L?' · LOAN '+L.id:'')+'</div><h2>'+name+'</h2><p>Record a reviewer disposition. This changes the review state, not the underlying evidence or policy evaluation.</p></div><span class="status wait"><i></i>Open evaluation</span></div><div style="padding:18px 19px"><div class="settingnote" style="margin-top:0"><b>Required record:</b> disposition, rationale, reviewer and timestamp. For production, these would be stored in an auditable backend record.</div><div style="margin-top:16px"><label style="display:block;font-size:11px;font-weight:900;margin-bottom:6px">Disposition</label><select id="findingDisposition" style="width:100%;border:1px solid #dfe4eb;border-radius:8px;padding:10px 11px;font-size:14px;font-weight:800"><option value="">Select disposition</option>'+opts+'</select></div><div style="margin-top:13px"><label style="display:block;font-size:11px;font-weight:900;margin-bottom:6px">Rationale</label><textarea id="findingRationale" rows="4" placeholder="Explain the reviewer decision, exception basis, or why the finding is being dismissed." style="width:100%;border:1px solid #dfe4eb;border-radius:8px;padding:10px 11px;font:inherit;font-size:12px;resize:vertical">'+rationale.replace(/</g,'&lt;').replace(/>/g,'&gt;')+'</textarea></div><div class="actions"><button class="primary" id="saveDisposition">Save disposition</button><a class="secondary" href="'+(L?'loan.html?loan='+L.id+'&station='+(role==='station1'?'1':'2'):'#'+role)+'">Cancel</a></div></div></section></main>';
 document.getElementById('saveDisposition').onclick=function(){
   var status=document.getElementById('findingDisposition').value, why=document.getElementById('findingRationale').value.trim();
   if(!status){alert('Select a disposition.');return;}
   if((status==='Waived'||status==='Escalated')&&!why){alert('Rationale is required for waived or escalated findings.');return;}
   if(!why){alert('Please add a short rationale for the reviewer decision.');return;}
   if(!L){alert('Loan context is missing.');return;}
   persistDecision(L.id,title,name,status,why);
   location.hash='';
 };
}
function render(){
 ...
 if(path.endsWith('/loan.html')){
   var sid=params.get('station'); if(sid==='2') role='station2'; else role='station1';
   var qloan=params.get('loan'); if(qloan && getLoan(qloan)){ if(!r){detail(getLoan(qloan));return;} } else if(!r){login();return;}
 }
